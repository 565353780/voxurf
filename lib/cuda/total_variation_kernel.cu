...
template <typename scalar_t, bool dense_mode>
__global__ void total_variation_add_grad_cuda_kernel_new(
    const scalar_t* __restrict__ param,
    scalar_t* __restrict__ grad, scalar_t* __restrict__ mask,
    float wx, float wy, float wz,
    const size_t sz_i, const size_t sz_j, const size_t sz_k, const size_t N) {
  ...
  if(index<N && (dense_mode || grad[index]!=0)) {
    ...
    float grad_to_add = 0;
    grad_to_add += (k==0      ? 0 : wx * clamp(param[index]-param[index-1], -1.f, 1.f) * mask[index] * mask[index-1]);
    grad_to_add += (k==sz_k-1 ? 0 : wx * clamp(param[index]-param[index+1], -1.f, 1.f) * mask[index] * mask[index+1]);
    grad_to_add += (j==0      ? 0 : wy * clamp(param[index]-param[index-sz_k], -1.f, 1.f) * mask[index] * mask[index-sz_k]);
    grad_to_add += (j==sz_j-1 ? 0 : wy * clamp(param[index]-param[index+sz_k], -1.f, 1.f) * mask[index] * mask[index+sz_k]);
    grad_to_add += (i==0      ? 0 : wz * clamp(param[index]-param[index-sz_k*sz_j], -1.f, 1.f) * mask[index] * mask[index-sz_k*sz_j]);
    grad_to_add += (i==sz_i-1 ? 0 : wz * clamp(param[index]-param[index+sz_k*sz_j], -1.f, 1.f) * mask[index] * mask[index+sz_k*sz_j]);
    // grad_to_add += (k==0      ? 0 : wx * clamp(param[index]-param[index-1], -1.f, 1.f)) * mask[index];
    // grad_to_add += (k==sz_k-1 ? 0 : wx * clamp(param[index]-param[index+1], -1.f, 1.f)) * mask[index];
    // grad_to_add += (j==0      ? 0 : wy * clamp(param[index]-param[index-sz_k], -1.f, 1.f)) * mask[index];
    // grad_to_add += (j==sz_j-1 ? 0 : wy * clamp(param[index]-param[index+sz_k], -1.f, 1.f)) * mask[index];
    // grad_to_add += (i==0      ? 0 : wz * clamp(param[index]-param[index-sz_k*sz_j], -1.f, 1.f)) * mask[index];
    // grad_to_add += (i==sz_i-1 ? 0 : wz * clamp(param[index]-param[index+sz_k*sz_j], -1.f, 1.f)) * mask[index];
    grad[index] += grad_to_add;
  }
}
...
void total_variation_add_grad_cuda_new(torch::Tensor param, torch::Tensor grad, torch::Tensor mask, float wx, float wy, float wz, bool dense_mode) {
  ...
  if(dense_mode) {
    AT_DISPATCH_FLOATING_TYPES(param.scalar_type(), "total_variation_add_grad_cuda_new", ([&] {
      total_variation_add_grad_cuda_kernel_new<scalar_t,true><<<blocks, threads>>>(
          param.data<scalar_t>(),
          grad.data<scalar_t>(),
          mask.data<scalar_t>(),
          wx, wy, wz,
     ...
     AT_DISPATCH_FLOATING_TYPES(param.scalar_type(), "total_variation_add_grad_cuda_new", ([&] {
      total_variation_add_grad_cuda_kernel_new<scalar_t,false><<<blocks, threads>>>(
          param.data<scalar_t>(),
          grad.data<scalar_t>(),
          mask.data<scalar_t>(),
          wx, wy, wz,
